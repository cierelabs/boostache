        ...
        namespace vm {
            namespace detail
            {
                ...
                public:
                    ...
                    engine_visitor_base(Stream & s, Context const & c, Stack const* stack)
                        : stream(s)
                        , context(c)
                        , stack(stack)
                    {}
                    ...
                    void operator()(ast::if_then_else const & v) const
                    {
                        ...
                        if (!result.second) {
                            ...
                            // do it again in the stack where element got found

                            boost::apply_visitor(boostache::detail::make_unwrap_variant_visitor<void>(
                                [this,&result,&v](auto ctx)
                            {
                                //Stack local(ctx, stack);
                                engine_visitor_base<Stream, decltype(ctx), Stack> engine(stream, ctx, stack);
                                //engine(v);
                                if (result.first)
                                {
                                    engine(v.then_);
                    ...
                    {
                        select_context_dispatch(stream, select_ctx, context, stack);
                    }
                ...
                {
                    engine_visitor_base<Stream, Context, Stack> engine(stream, ctx, stack);
                    engine(templ);
                }
