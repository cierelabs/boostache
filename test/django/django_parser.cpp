/**
 *  \file test/django/django_parser.cpp
 *
 *  Link with shared/parser_test to test the django parser
 *
...
#include <boost/boostache/frontend/django/grammar_def.hpp>
#include <boost/boostache/frontend/django/ast.hpp>
#include <boost/boostache/frontend/django/printer.hpp>
...
std::string print_ast(std::string const & filename)
{
   ...
   auto ast = fe::parse<bstache::format::django>(istream);
   std::ostringstream stream;
   fe::django::ast::print(stream,ast);
   return stream.str();
}


std::string test_dir = "django/parser_test_dir";
